...
*/
    if (isfree(addr)) {  /* check that it is on the free list */
      ...
      while (addr != free && free != TERMINATOR)
        free = fwdlink(free);
      if (free != addr) {
        sprintf(buf, "memchk: free block encountered not on chain %lld\n", addr);
        puts(buf);
        showfl();
        goto halt;
      }
      ...
      if (endptr(addr) < 0) {
        nprintf(OF_DEBUG_LOG, "memchk: endptr marker wrong on allocated block %lld\n", addr);
        goto halt;
      }
      if (blksize(addr) < minsize) {
        sprintf(buf, "memchk: small allocated block %lld of size %lld \n", addr, blksize(addr));
        wb(addr);
        goto halt;
      }
    }
